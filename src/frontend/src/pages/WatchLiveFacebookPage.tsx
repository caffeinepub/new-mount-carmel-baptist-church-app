...
export default function WatchLiveFacebookPage() {
  const { identity } = useInternetIdentity();
  const isAuthenticated = !!identity;
  
  const { data: livestream, isLoading } = useGetLivestream(LivestreamType.Facebook);
  ...
  const handleSave = async () => {
    ...
    const parsedEmbed = parseEmbedInput(embedInput, 'facebook');
    if (parsedEmbed) {
      await saveMutation.mutateAsync({
        type: LivestreamType.Facebook,
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">Facebook Live Stream</h1>
      ...
      {isEditing ? (
        <div className="bg-card border border-border rounded-lg p-6">
          <h3 className="text-lg font-semibold mb-4">Update Facebook Stream</h3>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-2">
                Facebook URL or Embed Code
              </label>
              <textarea
                value={embedInput}
                onChange={(e) => setEmbedInput(e.target.value)}
                placeholder="Paste Facebook video URL or embed code here..."
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Example: https://www.facebook.com/username/videos/VIDEO_ID or full embed code
              </p>
            </div>
        ...
        <div className="bg-card border border-border rounded-lg p-12 text-center">
          ...
          <p className="text-muted-foreground mb-6">
            {isAuthenticated
              ? 'Click "Edit Stream" to add a Facebook livestream.'
